...
Might not work correctly at this point

## Why Implement BASIC?

+ Having a script interpreter in a Godot game.
I want to have scripting portions in a game I'm planning, BASIC feels like it fit the retro aesthetics better than a more modern language.
It is also well documented and *basic* enough for a beginner to pick up.
+ Learn more about interpreters and parsing code.
...
PuffinBasicCS file ..\samples\prime.bas --list
...
1. Direct mode
1. Ready to use runtime that can be embedded in other projects and supports registering external methods
1. Android port, requires a rewrite of Graphics runtime.
	+ Perhaps through a .NET MAUI implementation
...
1. Use PuffinBasicCS.exe or PuffinBasicUIWinForms.exe to run the program.
...
Graphics currently uses WinForms, but the original project aims for cross-platform compatability.
The graphics runtime has been abstracted so other implementations can be dropped in.
