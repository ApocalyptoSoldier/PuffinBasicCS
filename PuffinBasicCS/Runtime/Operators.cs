namespace PuffinBasicCS.Runtime
{
    ...
    sealed class Operators
    {
        public static void UnaryMinus(PuffinBasicSymbolTable symbolTable, Instruction instruction)
        {
            ...
            switch (op1Entry.Type.AtomTypeId)
            {
                ...
                case DOUBLE:
                    ...
                    break;
                default:
                    throw new PuffinBasicInternalError("Unary minus is not supported for data type: " + op1Entry.Type.AtomTypeId);
                    //break;
            }
        }
        ...
        {
            var v1 = symbolTable[instruction.op1].Value.GetString();
            var v2 = symbolTable[instruction.op2].Value.GetString();
            var result = symbolTable[instruction.result].Value;
        ...
        public static void LeftShift(PuffinBasicSymbolTable symbolTable, Instruction instruction)
        {
            ...
            var v1 = v1Entry.Value;
            var v2Entry = symbolTable[instruction.op2];
            var v2 = v2Entry.Value;
            var result = symbolTable[instruction.result].Value;
            if (v1Entry.Type.AtomTypeId == INT32 && v2Entry.Type.AtomTypeId == INT32)
            {
        ...
        {
            var v1 = symbolTable[instruction.op1].Value.GetInt64();
            var v2 = symbolTable[instruction.op2].Value.GetInt64();
            var result = symbolTable[instruction.result].Value;
        ...
        {
            var e1 = symbolTable[instruction.op1].Value;
            var e2 = symbolTable[instruction.op2].Value;
            var result = symbolTable[instruction.result].Value;
        ...
        {
            var v = symbolTable[instruction.op1].Value.GetInt64();
            var result = symbolTable[instruction.result].Value;
            if (v == -1)
            {
                result.SetInt64(0);
